...
int main() {
	...
	int** primRoot = createGraph(end);
	int** kruskalRoot = createGraph(end);
	...
	Prim(root, start, end, primRoot);
	printGraph(primRoot, start, end);
	printf("\n");
	
	printf("Kruskal's Algorithm...\n");
	Kruskal(root, start, end, kruskalRoot);
	printGraph(primRoot, start, end);

	destroyGraph(root, end);
	destroyGraph(primRoot, end);
	destroyGraph(kruskalRoot, end);
}
